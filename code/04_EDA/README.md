...
* [`03_Explore_gene_level_effects.R`](03_Explore_gene_level_effects.R):
    * **3\.1 Analysis of explanatory variables**: obtain the contribution of each sample variable individually to gene expression variation.
        * *3\.1\.1 Computation of gene-wise expression variance percentages*: the percentages of variance in the expression of each gene explained by each individual sample variable were computed, including also rat behavioral variables. This was done for all habenula and amygdala samples separately, and then subsetting to fentanyl samples within each brain region group.
    ...
    * **3\.2 Variance Partition Analysis**: compute the fraction of gene expression variation attributable to each sample variable after correcting for all other variables.
        ...
        * *3\.2\.2 Model fit*: a linear mixed model (LMM) was fitted to the lognorm expression data of each gene to estimate the contribution in variance of each sample variable. This was performed for all habenula and amygdala samples separately, and for fentanyl samples from each brain region only. We did this including all sample variables and then subsetting to uncorrelated variables only.
